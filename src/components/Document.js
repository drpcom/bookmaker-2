...
const Document = ({_note, _index}) => {
  ...
  return (
      <>
    <button onClick={() => xSelectNote(_note, _index)} onMouseEnter={() => {setIsHovering(true)}} onMouseLeave={() => {setIsHovering(false)}} className='break-all bg-[#f3f4f6] dark:bg-zinc-700 rounded w-24 h-28 md:w-32 md:h-36 lg:w-44 lg:h-48 md:shadow-md md:shadow-slate-700/50 m-2 flex flex-col p-2 lg:p-3'>
        <div className='text-xl lg:text-2xl indent-2 flex flex-row w-full justify-between'>
            <p className='mb-2 line-clamp-1 w-[80%] h-6 md:h-8 text-left dark:text-sky-300 text-sm md:text-2xl'>{_note.title}</p> 
        ...
        <div>
           <p className='line-clamp-3 md:line-clamp-5 text-xs flex flex-row text-left lg:line-clamp-7 dark:text-slate-200'>{removeHTMLTags(_note.body.substring(0, 200)) + '...'}</p> 
        </div>    
    </button>
    </>
    );
